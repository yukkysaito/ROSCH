...
int SchedAnalyzer::run() {
  ...
  for (int i(0); (size_t)i < node_queue_.size(); ++i) {
    /* search vacancy core */
    //    for (int node_core(0); node_core < node_queue_.at(i).core;
    //    ++node_core) {
    ...
    if (node_queue_.at(i).core > 1) {
      ...
      while (true) {
        ...
        for (int node_core(0); node_core < node_queue_.at(i).core - 1;
             ++node_core) {
          ...
          get_sched_node_info_in_selected_cores(v_used_start_time.back(),
                                                ...
                                                v_can_use_core);
          int is_vacancy_node = get_sched_vacancy_node_info_in_selected_cores(
              node_queue_.at(i).run_time, v_used_start_time.back(),
      ...
      for (int node_core(0); node_core < node_queue_.at(i).core; ++node_core) {
        ...
        int is_vacancy_node = get_sched_vacancy_node_info_in_selected_cores(
            node_queue_.at(i).run_time, min_start_time,
            searched_sched_vacancy_node_info, v_can_use_core);
        if (is_vacancy_node == 1 &&
            (searched_sched_vacancy_node_info.start_time <=
             searched_sched_node_info.start_time)) {
          ...
          create_sched_node(node_queue_.at(i).index,
                            searched_sched_node_info.start_time,
