...
//  analyzer.show_tree_dfs();
  ...
  for (int i(0); i < core; ++i) {
    for (int j(0); (size_t)j < cpu_taskset.at(i).size(); ++j) {
      ...
      cv::rectangle(image, cv::Point(start_mag, core_line*(i+1) - core_line/2), cv::Point(end_mag, core_line*(i+1)), cv::Scalar(200,200,200), -1, 8);
      ...
      cv::putText(image, std::string(std::to_string(index)), cv::Point((start_mag+end_mag)/2-5, core_line*(i+1) - ((core_line/2) + 5)), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0,0,255), 1, CV_AA);
    }
  }
